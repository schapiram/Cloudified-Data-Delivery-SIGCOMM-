...
\vspace{0.1in}\noindent{\bf Experimental framework.} We used multiple settings to evaluate this simple strategy. In this section we will focus on three different routes; (1)~US East coast to Europe, (2)~US West coast to Mumbai, India, and (3)~US West coast to East coast. 
...
\vspace{0.1in}\noindent{\bf The simple strategy \emph{vs.} clever end-to-end congestion control.} Figure~\ref{fig:cloud-vs-CC} shows the difference in performance. In the continental US scenario, we can see that using the cloud provides x5 improvement over quick-start TCP, and x7 over BBR and PPC.

\vspace{0.1in}\noindent{\bf The simple strategy \emph{vs.} no connection splitting.} The need for splitting the TCP connections is demonstrated in figure~\ref{fig:must-split}. 
